...
import type { Category, RegistryPage } from '../payload-types'; // Import from local types
...
export const generateRegistryNavigation = async (payload: Payload): Promise<void> => {
  try {
    ...
    // 2. Fetch all published registry pages that are not section homepages
    const allPagesResult = await payload.find({
      collection: 'registry-pages',
      where: {
    ...
    const allPages = allPagesResult.docs as RegistryPage[];

    // 3. Create lookup maps for efficient tree building
    const pagesById: Map<string, RegistryPage> = new Map();
    const pagesByParentId: Map<string, RegistryPage[]> = new Map();
    const topLevelPagesByCategoryId: Map<string, RegistryPage[]> = new Map();
    ...
    if (existingCacheEntry.docs.length > 0) {
      await payload.update({
        ...
        data: {
          navigationData: navItems, // Use the processed navItems
        },
      });
    } else {
    ...
    payload.logger.error({ msg: 'Error in generateRegistryNavigation', err: error });
  }
};
